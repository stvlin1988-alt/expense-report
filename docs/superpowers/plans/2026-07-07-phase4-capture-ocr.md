...
## Addendum（最終 review 後補：Task 13 & 14）

> 最終全 branch review 發現原 12 tasks 漏做設計 §2.2.3（暫存區分類下拉）與 §2.2.6（無單據建帳 UI 入口）——後端皆完成、僅缺前端入口與員工用分類清單端點。user 決定現在補完（2026-07-07）。分類「先由 Gemini 自動分好、錯了讓員工/主管改」：自動分類已由 OCR prompt 完成，下拉的作用是**修正**（預選 OCR 建議、可改）。

### Task 13: 員工用分類清單端點 GET /expenses/categories

**Files:**
- Modify: `app/expenses/routes.py`
- Test: `tests/test_expense_categories.py`

**Interfaces:**
- Produces: `GET /expenses/categories` → `{status:"ok", categories:[{id, name, items:[{id, name}]}]}`（active，level1 科目為群組、其下 level2 項目為 items，皆依 sort）；未登入 401。

- [ ] **Step 1: Write the failing test**

```python
# tests/test_expense_categories.py
import time
from app.extensions import db
...
Run: `python3 -m pytest tests/test_expense_categories.py -v`
Expected: FAIL（404）

- [ ] **Step 3: Implement — append to `app/expenses/routes.py`**

```python
from app.models import Category  # 若檔頭尚未 import
...
```bash
git add app/expenses/routes.py tests/test_expense_categories.py
git commit -m "feat(expense): GET /expenses/categories 員工用 2 層分類清單"
```

---

### Task 14: 暫存區分類下拉（修正）+ 無單據建帳表單

**Files:**
- Modify: `app/static/js/expenses_util.js`（加純函式 `categoryOptionsHtml`）
- Modify: `app/static/js/expenses_api.js`（加 `listCategories`）
- Modify: `app/static/js/pending.js`（分類欄下拉 + [＋無單據建帳] 表單）
- Modify: `app/static/sw.js`（STATIC_URLS 已含這些檔；bump CACHE_NAME `calc-v11`→`calc-v12`）
- Test: `tests/js/expenses.mjs`（`categoryOptionsHtml`）

**Interfaces:**
- Produces（純函式，node 可測）：`categoryOptionsHtml(tree, selectedId) -> string`——回 `<optgroup>` 包 `<option>` 字串；首列固定 `<option value="">未分類</option>`；`selectedId` 對應的 option 加 `selected`；名稱以最小 regex escape（純函式、不依賴 DOM）。
- Consumes: `GET /expenses/categories`（Task 13）、`PATCH /expenses/<id>`（category_id）、`POST /expenses/no-receipt`（Task 10）。

- [ ] **Step 1: Write the failing JS test**

```javascript
// tests/js/expenses.mjs  （追加）
import { categoryOptionsHtml } from '../../app/static/js/expenses_util.js';
...
Run: `node --test tests/js/expenses.mjs`
Expected: FAIL（`categoryOptionsHtml` 未匯出）

- [ ] **Step 3: Implement pure helper（`app/static/js/expenses_util.js` 追加）**
...
- [ ] **Step 5: API wrapper（`app/static/js/expenses_api.js` 追加）**

```javascript
export const listCategories = () => jsonFetch('/expenses/categories');
```

- [ ] **Step 6: pending.js — 分類欄下拉 + 無單據建帳表單（DOM glue、手動 e2e）**

在 `showPendingView` 內：載入時先抓一次分類樹（`const { data: cat } = await listCategories(); const tree = cat.categories || [];`）。表格 header 於「摘要」「金額」之間插入 `<th>分類</th>`。每列在金額前插入分類欄：

```javascript
    const catSel = `<select data-f="category"><\/select>`;
    // ...組列後：
    const sel = tr.querySelector('[data-f="category"]');
    sel.innerHTML = categoryOptionsHtml(tree, e.category_id);
    sel.addEventListener('change', () => {
      const v = sel.value === '' ? null : Number(sel.value);
      patchExpense(e.id, { category_id: v });   // 即時修正、不阻塞送出
    });
```

送出時一併把當前分類帶入 PATCH（與 summary/amount 同批）：submit handler 內 `patchExpense(e.id, { summary, amount: parsed.value, category_id: sel.value === '' ? null : Number(sel.value) })`。
...
`pd-noreceipt` 點擊 → 於 modal-root 開一個表單（摘要 input／金額 input（用 `parseAmountInput` 驗）／分類 `<select>`（`categoryOptionsHtml(tree, null)`）／原因 input）→ 送出呼叫 `noReceipt({ summary, amount: parsed.value, category_id, reason })`；成功（status 200）→ 關表單並重載 `showPendingView(onBack)`；金額無效或缺原因 → 顯示錯誤不送出。

- [ ] **Step 7: sw.js bump**

`app/static/sw.js`：`CACHE_NAME` `calc-v11`→`calc-v12`（四支 expenses JS 已在 STATIC_URLS）。
...
```bash
git add app/static/js/expenses_util.js app/static/js/expenses_api.js app/static/js/pending.js app/static/sw.js tests/js/expenses.mjs
git commit -m "feat(expense-ui): 暫存區分類下拉(修正OCR建議) + 無單據建帳表單"
```
