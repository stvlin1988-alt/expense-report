...
| 路由 | 方法 | 用途 | Gate 行為 |
|---|---|---|---|
| `/` | GET | 計算機終端（唯一可見頁）。伺服器注入 `seed_mode`、`secret_hash`、（若已登入）`current identity` | **豁免** device/idle gate |
...
- **6 秒窗**：計算機**開啟（頁面載入）起算 6 秒**。若未在窗內完成暗號 → 本次工作階段**鎖定觸發**（`triggerLocked = true`），之後純為一般計算機、無法叫出系統；**須重整 / 重開** PWA 才能再試。
  - 6 秒起算點 = 頁面載入時（本 plan 定案；若日後要改「首次按鍵起算」再調）。
- **暗號混淆**：`078*2` 以伺服器注入的**鹽化 hash**（`secret_hash`，來源 config/env）比對；原始碼看不到明碼。定位為輕度混淆，真正安全靠裝置閘＋密碼＋人臉三層，不依賴暗號保密。
